 ...
 * icons are added to every tabbedpanel2, typically to let the user close or detach the
 * tabbedpanel2 by clicking on these icons.
 * 
...
public class CloseTabbedPane extends JTabbedPane {
	...
	/**
	 * Returns the index of the last tabbedpanel2 on which the mouse did an action.
	 */
	...
	/**
	 * Detaches the <code>index</code> tabbedpanel2 in a seperate frameanimation. When the frameanimation
	 * is closed, the tabbedpanel2 is automatically reinserted into the tabbedPane.
	 * 
	 * @param index
	...
	public void detachTab(int index) {
		 ...
		 * the time the new frameanimation would just blink from foreground to
		 * background). A windowFocusListener is added to the frameanimation, and if the
		 * time between the frameanimation beeing in foreground and the frameanimation beeing in
	 ...
	 * is received from the UI when the user clicks on the close icon of the tabbedpanel2
	 * which index is <code>overTabIndex</code>.
	 * 
	 * @param e
	 *            the <code>MouseEvent</code> to be sent
	 * @param overTabIndex
	 *            the index of a tabbedpanel2, usually the tabbedpanel2 over which the mouse is
	 * 
	 * @see #addCloseListener
	 ...
	 * <code>overTabIndex</code> of the tabbedPane with a value coming from
	 * the UI. This method method is called each time a <code>MouseEvent</code>
	 * is received from the UI when the user clicks on the max icon of the tabbedpanel2
	 * which index is <code>overTabIndex</code>.
	 * 
	 * @param e
	 *            the <code>MouseEvent</code> to be sent
	 ...
	 * <code>overTabIndex</code> of the tabbedPane with a value coming from
	 * the UI. This method method is called each time a <code>MouseEvent</code>
	 * is received from the UI when the user double-clicks on the tabbedpanel2 which
