...
public class IOandNewIO {
	...
	static void fileAttributes(Path path) throws IOException {
		...
		Files.setOwner(path, FileSystems.getDefault().getUserPrincipalLookupService().lookupPrincipalByName("aloskuto"));
	...
	static void directoryStream(Path dir) throws IOException {
		...
		Filter<? super Path> filter = new Filter<Path>() {
			...
			public boolean accept(Path path) throws IOException {
				return globMatcher.matches(path) && regexMatcher.matches(path) && !Files.isSymbolicLink(path);
			}
		};
	...
	private static final class MySimpleFileVisitor implements FileVisitor<Path> {
		...
		public FileVisitResult visitFileFailed(Path file, IOException exc) throws IOException {
			// surprise: the line below is not printed even if visitFile()
			// throws exception
