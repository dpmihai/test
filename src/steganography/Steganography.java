...
public class Steganography {
	 ...
	 * 
	 * A bit of message is put in the last bit of the current byte of image
	 * 
	...
	private byte[] encode_text(byte[] image, byte[] addition, int offset) {
		...
		for (int i = 0; i < addition.length; ++i) {
			...
			{
				// assign an integer to b, shifted by bit spaces AND 1 a single bit of the current byte
				int b = (add >>> bit) & 1;				
